...
class BlockExtractor:
    ...
    def get_page_count_static(pdf_path: str) -> int:
        """Get total number of pages without needing an instance."""
    ...
    def extract_from_page(
        ...
        """
        Extract all content blocks from a single page of an already-open document.

        Used by the background worker for page-level checkpointing.
        Keeps the PDF open across pages for efficiency.

        Args:
            doc: An open fitz.Document.
            page_num: 1-indexed page number.
            global_order: Starting order index for blocks on this page.

        Returns:
            List of ContentBlock objects for this page.
        """
        page_idx = page_num - 1
        ...
        with fitz.open(pdf_path) as doc:
            ...
            for page_idx in range(start_page - 1, end_page):
                ...
                if progress_callback:
                    ...
                    progress_callback(page_num - start_page + 1,
    ...
    def _extract_images_from_page(
        ...
        if len(page_images_raw) > 2000:
            logger.warning(
                f"Page {page_num} has {len(page_images_raw)} images. Skipping to avoid overload.")
            return []
        ...
        for img_idx, img in enumerate(page_images_raw):
            ...
            try:
                ...
                if img_hash not in self._image_hashes:
                    self._image_hashes[img_hash] = {
                        "rel_path": None, "count": 0}
                ...
                self._image_cache[xref] = {
                    "rel_path": rel_path, "is_logo": False}
                ...
                logger.warning(
                    f"Failed extracting image {xref} on page {page_num}: {e}")
